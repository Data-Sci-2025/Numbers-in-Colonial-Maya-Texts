...
* I created a GitHub repository and cloned it to my computer. I created project_plan and progress_report documents. I refined my project plan to include more information about my data, how I plan to break it down, and what kinds of packages I anticipate using.
...
* I opted to use the ["GNU GPLv3" license](LICENSE.md) because it is important to me that anyone who builds on the work in this project does so in a way that is accessible to as many people as possible with minimal barriers. I believe any work done with Maya data should be widely available to members of the Maya community.

## Progress report 3, 11/20/2025

* I chose to build onto my existing quarto document.
* The license and sharing scheme were addressed in Progress Report 2, and my data is in its ready-to-share form.
* I cleaned up and reformatted portions of this document to make it more consistent.
* I [extracted](data/ebtun_sample.pdf) and [read in](Numbers-in-Colonial-Maya-Texts.md#data-read-in) a much larger data set, now 200 pages of the original PDF, and rewrote the [row-cleaning](Numbers-in-Colonial-Maya-Texts.md#row-cleaning) chunk to eliminate all of the easily identified issues with the new data (I really can't emphasize enough how much work this was). My data is now in its final form.
* The previous analysis portion was not working. It was identifying a lot of false positives for numbers in the gloss column and it was struggling to identify Spanish numbers in the breakdown column because the spelling was not standardized. I added alternate spellings to the [maya_numbers](data/maya_numbers.csv) and [esp_numbers](data/esp_numbers.csv) CSVs and rewrote the [analysis](Numbers-in-Colonial-Maya-Texts.md#analysis) to first single out actual numbers in the gloss column and then correlate them with Maya and Spanish numbers from the CSVs found in the breakdown column. I also added a column to identify numerals in the breakdown column. I am now somewhat confident that I have identified all of the numbers in the data set. I then created a data frame called "number_counts" which tells how many numerals, Maya numbers, and Spanish numbers are in the data, just to make it easier to see what we're working with; right now I am sure Spanish numbers are over-represented because there tend to be several words involved in longer years (e.g. "mil setesyentos y onse" for 1711, four words to one number). This may also be the case for Maya. Over-representation isn't a significant problem, though, because frequency of use is not one of the things I will be looking at in my final analysis.
* **Next steps:**
  * Now that I have the data finalized and the numbers located, all I need to do is look at how they are used and when they are paired with numeral classifiers. My plan there is to create n-grams around the located numbers to get a sense of the context they are being used. Some numeral classifiers are already visible in the same word as the numbers (e.g. "ka'a-p'éel", meaning "two things") but I will also be looking for any that may follow numbers as separate words, to see which form is observed with numbers from which language.
